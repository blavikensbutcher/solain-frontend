      ...
      <Dialog open={!!selectedExercise} onOpenChange={(open) => !open && setSelectedExercise(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            {selectedExercise && getExerciseHistory(selectedExercise).map((workout, idx, arr) => {
              const currentDate = new Date(workout.account.timestamp.toNumber() * 1000);
              const showSeparator = idx > 0;
              const prevDate = showSeparator 
                ? new Date(arr[idx - 1].account.timestamp.toNumber() * 1000)
                : null;
              
              const showDateSeparator = showSeparator && prevDate && 
